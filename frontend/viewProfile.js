...
document.getElementById("branch").innerText=user.branch || "Student";

document.getElementById("topName").innerText=user.name;
document.getElementById("topBranch").innerText=user.branch || "Student";
...
if (user.lastInterviewScore) {
  const scoreDisp = document.getElementById("interviewScoreDisplay");
  const roleDisp = document.getElementById("interviewRoleDisplay");
  const dateDisp = document.getElementById("interviewDateDisplay");
  if (scoreDisp) scoreDisp.innerText = user.lastInterviewScore;
  if (roleDisp && user.interviewHistory && user.interviewHistory[0]) {
    roleDisp.innerText = `Target Role: ${user.interviewHistory[0].role}`;
  }
  if (dateDisp) dateDisp.innerText = `Last Attempted: ${user.lastInterviewDate || 'Recently'}`;
}
...
if(!user.skills.includes(val)) {
  user.skills.push(val);
...
if(!user.concepts.includes(val)) {
  user.concepts.push(val);
  localStorage.setItem("user",JSON.stringify(user));
}
...
alert("Career goal updated to: " + goal);
}
